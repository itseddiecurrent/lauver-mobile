...
function FilterChipGroup({ options, active, onToggle, styles, c }) {
  return (
...
function AthleteCard({ athlete, onPass, onLike, onStar, styles, c }) {
  const name    = athlete.display_name || 'Athlete';
  const initial = name.charAt(0).toUpperCase();
  const color   = avatarColor(name);
  const sports  = (athlete.sports || []).map(s => s.charAt(0).toUpperCase() + s.slice(1));
      ...
      <View style={styles.cardBody}>
        ...
        {sports.length > 0 && (
          <View style={styles.sportTags}>
            {sports.map(s => (
              <View key={s} style={[styles.sportTag, { backgroundColor: c.ELEVATED }]}>
        ...
        <View style={styles.cardDetails}>
          ...
          <View style={styles.cardDetail}>
            ...
            <Text style={styles.cardDetailVal} numberOfLines={1}>{(athlete.availability || []).join(' · ') || '—'}</Text>
          </View>
        </View>
      </View>
...
export default function MatchScreen({ navigation }) {
  ...
  const { readiness, isReady, candidates, matches, loading, sportFilters, setSportFilters, skillFilters, setSkillFilters, handleSwipe } = useMatch();
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.root}>
        <View style={styles.header}><Text style={styles.headerTitle}>FIND YOUR PACK</Text></View>
        <View style={styles.loadingWrap}><ActivityIndicator size="large" color={c.ORANGE} /></View>
      </SafeAreaView>
    );
  }
      ...
      <SafeAreaView style={styles.root}>
        ...
        <ScrollView contentContainerStyle={styles.gateScroll}>
          <View style={styles.gateCard}>
            ...
            <Text style={styles.gateSub}>Complete your profile to start finding training partners who match your sports, pace, and schedule.</Text>
  ...
  return (
    <SafeAreaView style={styles.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.unlockedScroll}>
        ...
        {currentAthlete ? (
          <AthleteCard athlete={currentAthlete} onPass={handlePass} onLike={handleLike} onStar={handleStar} styles={styles} c={c} />
        ) : (
        ...
        {filtersOpen && (
          <View style={styles.filtersPanel}>
            ...
            <FilterChipGroup options={SPORT_FILTERS} active={sportFilters} onToggle={o => toggleItem(sportFilters, setSportFilters, o)} styles={styles} c={c} />
            <Text style={styles.filterSectionLabel}>SKILL LEVEL</Text>
            <FilterChipGroup options={SKILL_FILTERS} active={skillFilters} onToggle={o => toggleItem(skillFilters, setSkillFilters, o)} styles={styles} c={c} />
            <Text style={styles.filterSectionLabel}>AVAILABILITY</Text>
            <FilterChipGroup options={AVAIL_FILTERS} active={avails} onToggle={o => toggleItem(avails, setAvails, o)} styles={styles} c={c} />
          </View>
        )}
        ...
        <View style={styles.mutualCard}>
          ...
          {matches.length === 0 ? (
            ...
            <View style={styles.mutualAvatars}>
              ...
              {matches.length > 5 && (
                <View style={[styles.mutualAvatar, { backgroundColor: c.DIVIDER }]}>
                  <Text style={[styles.mutualAvatarText, { color: c.TEXT_MUTED }]}>+{matches.length - 5}</Text>
                </View>
              )}
            </View>
          )}
...
function makeStyles(c) {
  return StyleSheet.create({
    ...
    gateTitle:  { fontSize: 22, fontWeight: '900', color: c.TEXT, marginBottom: 8, textAlign: 'center' },
    gateSub:    { fontSize: 14, color: c.TEXT_SUB, lineHeight: 20, textAlign: 'center', marginBottom: 20 },
    checklist:  { alignSelf: 'stretch', gap: 10, marginBottom: 24 },
    checkRow:   { flexDirection: 'row', alignItems: 'center', gap: 10 },
    checkDot:   { width: 22, height: 22, borderRadius: 11, borderWidth: 2, borderColor: c.DIVIDER, justifyContent: 'center', alignItems: 'center' },
    checkDotDone:  { backgroundColor: '#3A7D44', borderColor: '#3A7D44' },
    checkMark:     { color: '#fff', fontSize: 12, fontWeight: '900' },
    checkLabel:    { fontSize: 14, color: c.TEXT_MUTED },
    checkLabelDone:{ color: c.TEXT, fontWeight: '600' },
    completeBtn:   { backgroundColor: c.ORANGE, borderRadius: 14, paddingVertical: 14, alignSelf: 'stretch', alignItems: 'center' },
    completeBtnText:{ color: '#fff', fontSize: 15, fontWeight: '800' },

    filterToggle:           { borderWidth: 1.5, borderColor: c.DIVIDER, borderRadius: 20, paddingHorizontal: 14, paddingVertical: 6 },
    filterToggleActive:     { backgroundColor: c.TEXT, borderColor: c.TEXT },
    filterToggleText:       { fontSize: 13, fontWeight: '600', color: c.TEXT_SUB },
    filterToggleTextActive: { color: c.BG },
    unlockedScroll: { paddingBottom: 40 },
    matchSub:       { fontSize: 13, color: c.TEXT_MUTED, textAlign: 'center', marginBottom: 12, marginHorizontal: 16 },
    ...
    cardLocation:   { fontSize: 13, color: c.TEXT_MUTED, marginTop: 2, marginBottom: 12 },
    sportTags:      { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginBottom: 16 },
    sportTag:       { borderRadius: 12, paddingHorizontal: 10, paddingVertical: 4 },
    sportTagText:   { fontSize: 12, fontWeight: '600', color: c.TEXT_SUB },
    cardDetails:    { flexDirection: 'row', alignItems: 'center' },
    cardDetail:     { flex: 1, alignItems: 'center' },
    cardDetailLabel:{ fontSize: 9, fontWeight: '700', color: c.DARK_ORANGE, letterSpacing: 0.6 },
    cardDetailVal:  { fontSize: 12, fontWeight: '800', color: c.TEXT, marginTop: 3 },
    cardDetailDivider: { width: 1, height: 32, backgroundColor: c.DIVIDER },

    swipeRow:        { flexDirection: 'row', justifyContent: 'center', gap: 20, paddingVertical: 16, borderTopWidth: 1, borderTopColor: c.DIVIDER },
    swipeBtn:        { width: 56, height: 56, borderRadius: 28, justifyContent: 'center', alignItems: 'center' },
    swipeBtnPass:    { backgroundColor: c.ELEVATED, borderWidth: 2, borderColor: '#FF6B6B' },
    swipeBtnStar:    { backgroundColor: c.ELEVATED, borderWidth: 2, borderColor: '#FFD700' },
    ...
    emptyCard:  { backgroundColor: c.CARD_BG, borderRadius: 16, marginHorizontal: 16, padding: 32, alignItems: 'center', marginBottom: 12 },
    ...
    filtersPanel:       { backgroundColor: c.CARD_BG, borderRadius: 16, marginHorizontal: 16, padding: 16, marginBottom: 12 },
    filterSectionLabel: { fontSize: 11, fontWeight: '800', color: c.DARK_ORANGE, letterSpacing: 1, marginBottom: 8, marginTop: 4 },
    chipGroup:      { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 12 },
    chip:           { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20, backgroundColor: c.DIVIDER },
    chipActive:     { backgroundColor: c.TEXT },
    chipText:       { fontSize: 12, fontWeight: '600', color: c.TEXT_SUB },
    chipTextActive: { color: c.BG },

    mutualCard:       { backgroundColor: c.CARD_BG, borderRadius: 16, marginHorizontal: 16, padding: 16, marginBottom: 12 },
    mutualTitle:      { fontSize: 14, fontWeight: '800', color: c.TEXT, marginBottom: 4 },
    mutualSub:        { fontSize: 12, color: c.TEXT_MUTED, marginBottom: 4 },

...
/*
  Supabase — add photos column if not present:
  alter table profiles add column if not exists photos text[] default '{}';
*/

const { width: SW, height: SH } = Dimensions.get('window');
const SLOT_SIZE = (SW - 32 - 8) / 3;   // 3-column grid with gap
const MAX_PHOTOS = 6;
...
// ─── Photo Grid ───────────────────────────────────────────────────────────────
// Tap a filled slot to select it (orange border). Tap another filled slot to
// swap. Tap an empty slot while one is selected to move the photo there.
// Tap ✕ to remove. Tap + on empty slot (none selected) to add a new photo.

function PhotoGrid({ photos, uploadingSlot, selectedSlot, onAdd, onRemove, onSelect, onSwapOrMove }) {
        ...
        return (
          <TouchableOpacity
            ...
            onPress={() => {
              if (loading) return;
              if (filled) {
                // If another slot is selected, swap/move
                ...
                // Empty slot: if something selected, move there; else add new
...
export default function ProfileScreen() {
  ...
  // Photos
  const [photos,        setPhotos]        = useState([]);   // array of URLs, max 6
  const [uploadingSlot, setUploadingSlot] = useState(null); // index uploading
  const [selectedSlot,  setSelectedSlot]  = useState(null); // index selected for reorder
      ...
      <Modal visible={showPreview} animationType="slide" statusBarTranslucent onRequestClose={() => setShowPreview(false)}>
        <View style={styles.modalRoot}>
          ...
          <View style={styles.previewBanner}>
            <Text style={styles.previewBannerText}>👀  THIS IS YOUR PREVIEW</Text>
          </View>
      ...
      <Modal visible={showSettings} animationType="slide" onRequestClose={() => setShowSettings(false)}>
        <SafeAreaView style={styles.settingsRoot}>
          ...
          <ScrollView contentContainerStyle={styles.settingsScroll} showsVerticalScrollIndicator={false}>
            ...
            {[
              { key: 'apple',  name: 'Apple Health',    emoji: '❤️', bg: '#FF2D55', desc: 'Sync workouts, heart rate, and activity rings from your iPhone and Apple Watch.', connected: appleConnected,  setConnected: setAppleConnected  },
              { key: 'garmin', name: 'Garmin Connect',  emoji: '⌚', bg: '#007CC3', desc: 'Import runs, rides, swims and all activities recorded on your Garmin device.',    connected: garminConnected, setConnected: setGarminConnected },
              { key: 'strava', name: 'Strava',          emoji: '🚴', bg: '#FC4C02', desc: 'Pull in all your Strava activities and keep your training log in sync.',           connected: stravaConnected, setConnected: setStravaConnected },
            ].map(app => (
              <View key={app.key} style={styles.integrationCard}>
                <View style={[styles.integrationIcon, { backgroundColor: app.bg }]}>
                  <Text style={styles.integrationEmoji}>{app.emoji}</Text>
                </View>
                <View style={styles.integrationInfo}>
                  ...
                  {app.connected && (
                    <View style={styles.integrationStatus}>
                      ...
                      <Text style={styles.statusText}>Connected · Syncing</Text>
                    </View>
                  )}
                </View>
                <TouchableOpacity
                  style={[styles.connectBtn, app.connected && styles.connectBtnActive]}
                  onPress={() => app.setConnected(v => !v)}
                  activeOpacity={0.8}
                >
...
const styles = StyleSheet.create({
  ...
  photosHint:  { fontSize: 11, color: '#999', textAlign: 'center', marginBottom: 12 },
  photosCount: { fontSize: 11, color: '#AAA', textAlign: 'center', marginTop: 10 },
  ...
  photoSlotPlus:  { fontSize: 28, color: '#AAA', fontWeight: '300' },
  ...
  previewBtn: {
    ...
    alignItems: 'center', borderWidth: 1.5, borderColor: DARK,
  },
  previewBtnText: { fontSize: 14, fontWeight: '700', color: DARK },
  ...
  previewBanner: {
    backgroundColor: ORANGE, paddingHorizontal: 20, paddingVertical: 6,
  ...
  cardSkillBadge:    { backgroundColor: ORANGE, borderRadius: 10, paddingHorizontal: 10, paddingVertical: 3 },
  ...
  // ══ Settings Modal ════════════════════════════════════════════════════════════
  settingsRoot:       { flex: 1, backgroundColor: BG },
  settingsHeader:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#E0DAD3' },
  settingsTitle:      { fontSize: 20, fontWeight: '900', color: DARK },
  settingsClose:      { fontSize: 18, color: '#888', fontWeight: '700', padding: 4 },
  settingsScroll:     { padding: 20, paddingBottom: 40 },
  settingsSection:    { fontSize: 12, fontWeight: '900', color: DARK, letterSpacing: 1, marginBottom: 6 },
  settingsSectionSub: { fontSize: 13, color: '#888', lineHeight: 18, marginBottom: 20 },
  settingsNote:       { fontSize: 12, color: '#AAA', textAlign: 'center', marginTop: 8, lineHeight: 18 },

  integrationCard:   { backgroundColor: CARD_BG, borderRadius: 16, padding: 16, marginBottom: 12, flexDirection: 'row', alignItems: 'center', gap: 14 },
  integrationIcon:   { width: 48, height: 48, borderRadius: 14, justifyContent: 'center', alignItems: 'center', flexShrink: 0 },
  integrationEmoji:  { fontSize: 22 },
  integrationInfo:   { flex: 1 },
  integrationName:   { fontSize: 15, fontWeight: '800', color: DARK, marginBottom: 3 },
  integrationDesc:   { fontSize: 12, color: '#777', lineHeight: 17 },
  ...
  connectBtn:           { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1.5, borderColor: DARK, flexShrink: 0 },
  connectBtnActive:     { backgroundColor: DARK, borderColor: DARK },

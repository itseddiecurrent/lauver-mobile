...
**状态：🟡 核心实现与 Render Email smoke 已通过，完整验收待完成（2026-09-18）。** 删除编排、幂等 retry、会话立即撤销、外部清理接线、Email/Apple 重新认证、原生设置入口和 Simulator 构建已完成；Render staging 已验证注册→密码重新认证删除→旧 token 失效。真实 Apple + Strava provider cleanup、PostgreSQL/object storage/Stream 清理证据、完整原生测试及真机重启验证仍待完成。证据见 `artifacts/acceptance/step-14.md`。

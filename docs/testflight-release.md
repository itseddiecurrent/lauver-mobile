...
- `Lauver-Production` archive 已成功，bundle ID 为 `ai.lauver.app.release`，Team ID 为 `94KFUD562T`，HealthKit 和 Sign in with Apple entitlements 已包含。Transporter `90683` 校验后，已加入 HealthKit、Photo Library、Microphone 和 Camera 的所需 purpose strings；同时补齐 App Store 所需的 iPhone 与 1024px marketing App Icon entries。最终 build 为 `1.0.0 (4)`，`exportArchive` 已成功生成可上传 TestFlight 的 Distribution IPA：[`Lauver.ipa`](../artifacts/acceptance/testflight-build4-20260921/Lauver.ipa)，签名为 Apple Distribution，`beta-reports-active=true` 且 `get-task-allow=false`。
...
请上传 build `1.0.0 (4)` 的 IPA：[`artifacts/acceptance/testflight-build4-20260921/Lauver.ipa`](../artifacts/acceptance/testflight-build4-20260921/Lauver.ipa)。其 SHA-256 是 `d9f40fecbc68c00ea321f45892f7e4d6d69a2a1e9e0ddcfcab781648b291f7a4`，最终 bundle 包含 HealthKit、照片、麦克风和相机 purpose strings，以及完整的 App Icon catalog。可在 Xcode Organizer 选择 archive Validate App，也可直接在 Transporter/Xcode Organizer 中上传 IPA。上传完成后，在 App Store Connect 的 TestFlight 页面等待 processing，再先给内部测试组发放 build。

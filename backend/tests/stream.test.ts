...
describe('Stream chat ownership and canonical channels', () => {
  ...
  it('allows only server-mediated sends and protects channel membership', async () => {
    ...
    expect(sdk.updateChannelType).toHaveBeenCalledWith('messaging', expect.objectContaining({ grants: { user: [], guest: [], anonymous: [], channel_member: ['read-channel', 'read-channel-members', 'create-message'] } }));
